...
function calculateWorkHours(data) {
  ...
  const totalWorkHours = data.reduce((total, item) => total + item.workHours, 0);
...
<li v-if="isLoaded === true" v-for="workingtime in workingtimes">
  {{ Username }} - {{ workingtime.user.username }} - {{ UserID }} - {{ workingtime.user.id }} - {{ Clockin }} - {{ dateFormatee(workingtime.start) }} - {{ Clockout }} - {{ dateFormatee(workingtime.end) }} - {{ WorkTime }} - {{ workingtime.workHours }}
</li>
