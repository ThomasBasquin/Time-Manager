...
async function Login() {
    try {
        ...
        const response = await axios.get('http://157.230.19.191:4000/api/users/', {
            params: {
                email: store.email
            }
        })
...
async function Register() {
    try {
        ...
        const response = await axios.post('http://157.230.19.191:4000/api/users/', {
            user: {
                email: store.email,
    ...
    try {
        ...
        const response = await axios.post('http://157.230.19.191:4000/api/clocks/', {
            clock: {
                status: false,
                time: new Date(),
...
async function deleteUser() {
    try {
        ...
        const response = await axios.delete('http://157.230.19.191:4000/api/users/' + store.user.id)
        // Traiter la réponse de l'API
        const user = response.data
...
async function editUser() {
    try {
        ...
        const response = await axios.put('http://157.230.19.191:4000/api/users/' + store.user.id, {
            user: {
                email: store.email,
                username: store.username
            }
